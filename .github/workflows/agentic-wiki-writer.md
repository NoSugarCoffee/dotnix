...
tools:
  # The codex engine (required for OpenRouter, see the `engine:` comment above)
  # doesn't support tools.bash allow-listing -- gh-aw ignores a restricted list
  # silently at runtime for this engine, so the upstream sandboxed subset
  # (find/tree/wc/ls/cat/head only) can't be enforced here. The prompt's own
  # "CRITICAL: Sandbox constraints" section still tells the model what not to
  # run; matches bash: ["*"] in code-quality.md and weekly-issue-activity.md.
  bash:
    - "*"
  edit:
  repo-memory:
      ...
      steps:
        ...
        - name: Commit and push
          run: |
            ...
            git push
timeout-minutes: 30
source: githubnext/agentics/workflows/agentic-wiki-writer.md@578e0e0ea6291fed42a36d3fd46cec6a0e86afd8
---
