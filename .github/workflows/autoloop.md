...
on:
  # Upstream default is `every 6h`. Held at manual-only until this repo has a
  # program worth running -- with no programs defined, each firing would spin
  # up an agent, find nothing, and bill for it four times a day.
  workflow_dispatch:
    inputs:
      program:
...
network:
  ...
  - java
  - dotnet
  # The agent sits behind the AWF egress firewall, so the model provider has to
...
# Upstream ships engine: copilot. This repo has no Copilot token and runs its
# agents through OpenRouter instead -- see .github/workflows/code-quality.md,
# which uses the same wiring.
engine:
  id: codex
  env:
