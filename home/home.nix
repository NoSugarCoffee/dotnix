...
in
{
  ...
  # this file directly and re-switch instead. Keep `version` at ccstatusline's
  # CURRENT_VERSION (4 for 2.2.29): an older schema makes the binary migrate
  # and write a tempfile next to the store target, which EACCES's and shows
  # "⚠ invalid config" on every prompt. force: the TUI creates this path as
  # a regular file, and checkLinkTargets would otherwise refuse the first switch.
